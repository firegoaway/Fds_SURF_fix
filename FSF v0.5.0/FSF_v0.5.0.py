...
class FDSProcessorApp:
    def __init__(self, page: ft.Page):
        self.page = page
        self.page.title = f"FSF v0.5.0 ID:{ProcessID}"
    ...
    def calculate(self, e):
        ...
        try:
            ...
            # Проверяем, получилась ли Psy > 1 при m = 0
            if m == 0 and Psi > 1:
                # Пересчитываем m для Psy = 1
                new_m = ceil(tmax)  # Из уравнения Psi = m / tmax, выражаем m  при Psi = 1, получая m = tmax
                new_Psi = new_m / tmax  # Пересчитываем Psy

                # Обновляем HRRPUA всвязи с новым значением Psi
                HRRPUA = Hc * Psi * 0.93 * 1000

                # Отобразить уведомление в графическом интерфейсе с предложением ввести новое значение m
                snack_bar = ft.SnackBar(
                    content=ft.Text(f"Рекомендуемое значение m = {new_m:.0f} кг"),
                    #action="OK",
                    #action_color=ft.colors.GREEN,
                    duration=1000,  # Показываем уведомление 10 секунд
                )
