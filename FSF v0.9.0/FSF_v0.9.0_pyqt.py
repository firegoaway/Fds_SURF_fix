...
class FDSProcessorAppQt(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        header1.setFont(QFont("Segoe UI", 12, QFont.Weight.Bold))
        header1.setAlignment(Qt.AlignmentFlag.AlignCenter)
        header1.setStyleSheet("color: rgb(3, 105, 161); padding: 0px; margin: 0px;")
        header_layout.addWidget(header1)

        header2 = QLabel("(согласно Приложению 1 Методики 1140)")
        header2.setFont(QFont("Segoe UI", 9, QFont.Weight.Normal))
        header2.setAlignment(Qt.AlignmentFlag.AlignCenter)
        header2.setStyleSheet("color: rgb(3, 105, 161); padding: 0px; margin: 0px;")
        ...
        self.report_md_button.setFont(QFont("Segoe UI", 10, QFont.Weight.Light))
        self.report_md_button.setStyleSheet(get_button_style_common())
        self.report_md_button.setFixedWidth(190)
        ...
        self.report_docx_button.setFont(QFont("Segoe UI", 10, QFont.Weight.Light))
        self.report_docx_button.setStyleSheet(get_button_style_common())
        self.report_docx_button.setFixedWidth(190)
        ...
        input_layout.addWidget(q_mode_group)  # ComboBox режима Q
        input_layout.addWidget(self.aupt_checkbox)  # Чекбокс АУПТ
        input_layout.addWidget(aupt_input_container)  # Поле t_АУПТ
    ...
    def _on_q_mode_changed(self, index):
        ...
        index 0 = Расчетная MLR (Методика 1140) – показываем Q как обычно
        index 1 = Произвольный Q – скрываем/бледим поле Q с текстом 'Контролируется функцией Ψ(t)'
        """
        if index == 1:
            # Режим "Произвольный Q" – меняем label и прячем lineedit
            for lbl in self.hrr_labels:
                if lbl.text() == self.hrr_original_label_text:
                    ...
                    """)
                    break
            # Скрываем lineedit и добавляем placeholder QLabel
            ...
            if not existing_placeholder:
                ...
                """)
                # Вставляем после lineedit
                layout = self.hrr_entry[0].layout()
                layout.addWidget(placeholder)
        else:
            # Режим "Расчетная MLR" – восстанавливаем
            for lbl in self.hrr_labels:
                if lbl.text() == "Ψ(t)":
    ...
    def _on_aupt_changed(self, state):
        """Показываем/скрываем поле ввода t_АУПТ при изменении чекбокса."""
        # Находим родительский контейнер t_aupt_entry
