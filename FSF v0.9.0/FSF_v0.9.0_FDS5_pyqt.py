...
class FDSProcessorAppQt(QMainWindow):
    def __init__(self, process_id=None):
        ...
        self.setWindowTitle(f"FSF v0.9.0 FDS5 ID:{self.process_id if self.process_id is not None else 'N/A'}")
    ...
    def _setup_ui(self):
        ...
        header1.setFont(QFont("Segoe UI", 16, QFont.Weight.Bold))
        header1.setAlignment(Qt.AlignmentFlag.AlignCenter)
        header1.setStyleSheet("color: rgb(3, 105, 161); padding: 10px;")
        main_layout.addWidget(header1)

        header2 = QLabel("(согласно Приложению 1 Методики 1140)")
        header2.setFont(QFont("Segoe UI", 12, QFont.Weight.Bold))
        header2.setAlignment(Qt.AlignmentFlag.AlignCenter)
        header2.setStyleSheet("color: rgb(3, 105, 161); padding: 5px;")
