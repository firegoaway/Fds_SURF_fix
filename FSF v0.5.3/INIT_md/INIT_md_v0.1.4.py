...
class MainWindow(QMainWindow):
    def __init__(self, process_id=None):
        ...
        self.setWindowTitle(f"(\u03b1) Расчет СПДЗ v0.1.4 ID: {self.unique_id}")
    ...
    def calculate_and_plot(self, all_data, groups):
        ...
        for group, dev_ids in groups.items():
            ...
            density_avg_min = np.mean(density_mins) if density_mins else 0
            mflow_avg_max = np.mean(mflow_maxes) if mflow_maxes else 0
